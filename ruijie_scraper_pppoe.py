...
def jalankan_auto_login():
    print("[INFO] Mencoba login otomatis PPPoE untuk mendapatkan Cookie baru...")
    try:
        # Panggil script login
        subprocess.run(["python3", "ruijie_login_pppoe.py"], check=True)
        # Muat ulang cookie
        return load_cookies()
        ...
        print("[ERROR] ruijie_login_pppoe.py tidak ditemukan.")
        return False
...
PAYLOAD = {
    ...
    "currentUsername": os.getenv("RUIJIE_EMAIL_PPPOE")
}
...
def ambil_data_ruijie(putaran_pertama=False):
    ...
    try:
        ...
        print(f"\n[{waktu_sekarang}] Data PPPoE berhasil diambil ({len(all_ap_list)} perangkat)!")
        ...
        print(f"Total perangkat PPPoE: {total_records} (Online: {jumlah_online}, Offline: {jumlah_offline})")
        ...
        for ap in all_ap_list:
                    ...
                    network = ap.get('groupName') or ap.get('projectName') or ap.get('networkName') or 'TanpaNetwork'
                    if ' / ' in network:
                        network = network.split(' / ')[0]
                    alias_raw = ap.get('aliasName') or ap.get('name') or ap.get('hostName') or ap.get('remark') or 'TanpaAlias'
                    
                    alias = f"{network}-{alias_raw}".upper()
...
if __name__ == "__main__":
    print(f"Memulai monitoring Ruijie PPPoE setiap {INTERVAL_DETIK} detik...\nTekan Ctrl+C untuk berhenti.")
    try:
        ...
        while True:
            ...
            for s in range(INTERVAL_DETIK, 0, -1):
                sys.stdout.write(f"\rMenunggu tarikan PPPoE berikutnya dalam {s} detik... \033[K")
                sys.stdout.flush()
                time.sleep(1)
            sys.stdout.write("\r\033[K")
    except KeyboardInterrupt:
        print("\nMonitoring PPPoE dihentikan.")
